...
def train(model, x_train, y_train, x_valid, y_valid, config):
    ...
    #val_accs = np.zeros((num_epochs,1))
    #val_losses = np.zeros((num_epochs,1))
    ...
    for i in range(num_epochs):
        ...
        #val_accs[i] = np.sum(np.argmax(val_out, axis=1)==np.argmax(y_valid, axis=1))/N_val
